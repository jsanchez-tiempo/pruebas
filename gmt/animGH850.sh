...
titulo="Temperatura a 850hPa"
#titulo="CAPA DE 850HPA"
#titulo="TEMP. A 850HPA"
...
cptGMT="cpt/temp850.cpt"

mins=12

OUTPUTS_DIR="/home/juan/Proyectos/pruebas/gmt/OUTPUTS/"
outputFile="${OUTPUTS_DIR}/t850-meteored.mkv"
...
#    ncFile="${fecha}.nc"
#    variable="t"
    dataFileGH=${TMPDIR}/${fecha}_gh.nc
#    dataFile=${TMPDIR}/${fecha}_t.nc

    gmt grdfilter ${dataFileGH} -Fb19 -Dp -G${TMPDIR}/kk.nc

    printMessage "Generando frame de GH850 para fecha ${fecha}"
...
#    fecha=`date -u --date="${ti:0:8} ${ti:8:2}" +%Y%m%d%H%M`
    ...
    # Pintar las isotermas
    gmt grdcontour ${TMPDIR}/kk.nc -S100 -J -R -Q50000 -W0.6p,white -C4 -K -O >> ${tmpFile}

    # Pintar los puntos rotulos
    gmt grdcontour ${dataFileGH} -J -R -A4+t"${TMPDIR}/contourlabels.txt" -Q500 -Gn1 > /dev/null
    awk '{if ($4!="H" && $4!="L") print $1,$2,$4}' ${TMPDIR}/contourlabels.txt |  gmt pstext -J -R -F+f10p,Helvetica-Bold,black+a0  -G${color}  -K -O >> ${tmpFile}
    ...
    cp ${outputpng} ${TMPDIR}/fgh`printf "%03d\n" ${nframe}`.png


    tmpPNG="${TMPDIR}/fgh`printf "%03d\n" ${nframe}`.png"


    ffmpeg -f image2 -i ${TMPDIR}/kkb-%03d.png -i  ${tmpPNG} -filter_complex "[1]loop=20:1:0[gh];[gh]fade=in:0:10[gh];[gh]setpts=PTS+${nframe}/(25*TB)[gh];[0][gh]overlay" -y -c:v png -f image2 ${TMPDIR}/kkc-%03d.png #2>> ${errorsFile}
...
zmin=99999
zmax=-1
printMessage "Procesando los grids de Temperatura a 850hpa (T) desde ${fecha} hasta ${fechamax}"
...
#    echo "RGeog: ${Rgeog}"
    ...
    cdo -sellevel,850 -selvar,t ${TMPDIR}/${fecha}.nc ${dataFile} 2>> ${errorsFile}

    gmt grdconvert -Rd ${dataFile}\?t ${dataFile} 2>> ${errorsFile}
    gmt grdmath ${Rgeog} ${dataFile} 273.15 SUB = ${dataFile} 2>> ${errorsFile}
    ...
    read zminlocal zmaxlocal < <(gmt grdinfo ${dataFile} -C | awk '{printf "%.0f %.0f\n",$6-0.5,$7+0.5}')
...
./crearescala.sh ${zmin}/${zmax} ${cptGMT} ${TMPDIR}/escala.png "°C" #2>> ${errorsFile}




#### Altura geopotencial último grid
printMessage "Procesando grid de Altura Geopotencial (GH) para ${max}"

ncFile="${TMPDIR}/${max}.nc"
variable="gh"
dataFileGH=${TMPDIR}/${max}_${variable}.nc
dataFile=${TMPDIR}/${max}_t.nc

cdo -sellevel,850 -selvar,${variable} ${ncFile} ${dataFileGH} 2>> ${errorsFile}
gmt grdconvert -Rd  ${dataFileGH}?${variable} ${dataFileGH} 2>> ${errorsFile}
#    gmt grdconvert -Rd  ${ncFile}?${variable} ${dataFileT}
#Rgeog=${R}
RGeog
gmt grdmath ${Rgeog} ${dataFileGH} 9.81 DIV 10 DIV = ${dataFileGH} 2>> ${errorsFile}
gmt grdproject ${dataFileGH}  ${R} ${J} -G${dataFileGH} 2>> ${errorsFile}
...
do
    ...
    fecha=`date -u --date="${ti:0:8} ${ti:8:2}" +%Y%m%d%H%M`
    tfile=${TMPDIR}/${fecha}_t.nc

    tmpFile="${TMPDIR}/${fecha}-t.ps"
...
printMessage "Insertando animación de GH850"
pintarGH850 ${max} $((${nframe}+${nframesloop}))
...
ffmpeg -f image2 -i ${TMPDIR}/kkc-%03d.png -i ${TMPDIR}/rotulo-%03d.png -i  ${TMPDIR}/escala.png -filter_complex "[2]loop=20:1:0[escala];[escala]fade=in:0:10[escala];[0][1]overlay[out];[out][escala]overlay=x=${xscala}:y=${yscala}[out];[out]loop=10:1:$((${nframes}-1))" -y -c:v png -f image2 ${TMPDIR}/kkd-%03d.png 2>> ${errorsFile}
